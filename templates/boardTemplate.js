...
function renderOverlayTicket(category, ticketTitle, ticketDescription, ticketDate, prio, ticketID) {
  return `<div class="overlay-card slide-in-right" id="overlayCard">
            ...
            <div class="overlay-service-btn-wrapper">
                ...
                <div class="overlay-ticket-edit" onclick="editTicket('${ticketID}', '${ticketTitle}', '${ticketDescription}', '${ticketDate}', '${prio}'); renderSubtasksEditOverlay('${ticketID}'); updateSelectedUsersContainer()">
                    <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function showContextMenu(event, ticketID) {
  ...
  menu.innerHTML = `<ul>
        <li onclick="moveTicket('${ticketID}', 'toDo')">Move to To Do</li>
        <li onclick="moveTicket('${ticketID}', 'inProgress')">Move to In Progress</li>
        <li onclick="moveTicket('${ticketID}', 'awaitingFeedback')">Move to Awaiting Feedback</li>
        <li onclick="moveTicket('${ticketID}', 'done')">Move to Done</li>
    </ul>`;
...
function moveTicket(ticketID, status) {
  console.log(`Ticket ${ticketID} moved to ${status}`);
  closeContextMenu();
}

let longPressTimeout;

function setupLongTap(ticketID) {
  const ticketElement = document.querySelector(`#ticket_${ticketID}`);

  ticketElement.addEventListener("mousedown", (e) => {
    longPressTimeout = setTimeout(() => {
      showContextMenu(e, ticketID);
    }, 800); // 800ms für Long-Tap
  });

  ticketElement.addEventListener("mouseup", () => {
  ...
  ticketElement.addEventListener("touchstart", (e) => {
    longPressTimeout = setTimeout(() => {
      showContextMenu(e.touches[0], ticketID);
    }, 800);
  });

  ticketElement.addEventListener("touchend", () => {
    clearTimeout(longPressTimeout);
  });
}
